...
export function authenticate(req, res, next) {
    ...
    if (type !== "Bearer" || !token || extra) return res.status(401).json({ message: "Invalid authorization format" });
    try {
        const payload = verifyAccessToken(token);
        if (!payload.sub) throw new Error("Token subject is required");
        req.auth = { userId: payload.sub, email: payload.email, role: payload.role || "user", accessToken: token };
        return next();
    } catch {
